...
export const LineFlag = () => {
  ...
  })) as { humidity: number; timestamp: Date }[];
  const pressureData = data.map((item) => ({
    pressure: item.pressure,
    timestamp: item.timestamp,
  })) as { pressure: number; timestamp: Date }[];
  const temperatureData = data.map((item) => ({
    temperature: item.temperature,
    timestamp: item.timestamp,
  })) as { temperature: number; timestamp: Date }[];
  ...
  const chart = (result: { timestamp: Date; value: number }[]) => {
    const svg = d3
    ...
    const xScale = d3
      .scaleTime()
      .domain(d3.extent(result, (d) => d.timestamp) as [Date, Date])
      .range([0, width]);
    ...
    g.append("g")
      ...
      .text(function (d) {
        if (Object?.values(d)[0] || 0 > 0) {
          if (Object?.values(d)[0] || 0 > 50) {
            return "\uD83D\uDC4D";
          }
          return "\uD83D\uDC4E";
        }
  ...
  return (
    <div>
      ...
      <button
        onClick={() => setResult(temperatureData)}
        style={{ backgroundColor: "red", padding: "14px", color: "white" }}
      >
        Temperature
      </button>
      <button
        onClick={() => setResult(humidityData)}
        style={{ backgroundColor: "blue", padding: "14px", color: "white" }}
      >
        Humadity
      </button>
      <button
        onClick={() => setResult(pressureData)}
        style={{ backgroundColor: "grey", padding: "14px", color: "white" }}
      >
        Pressure
      </button>
    </div>
